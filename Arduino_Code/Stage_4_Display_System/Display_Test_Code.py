...
// ============================================================
// SMART MIC FINAL VERSION
// A0 = pre-filter signal
// A1 = post-filter signal going to TDA2822 speaker amp
// D2/D3/D4 = LOW/MID/HIGH transistor filter control
// ============================================================

GigaDisplay_GFX display;

// ---------------- SPEAKER / FILTER TEST MODE ----------------
// 0 = normal automatic mode
// 1 = force LOW/MID/HIGH every 3 seconds so you can hear the output change
#define SPEAKER_TEST_MODE 0

// ---------------- Pins ----------------
#define PRE_FILTER_PIN   A0
#define POST_FILTER_PIN  A1
...
const double LOW_TO_MID_FREQ  = 400.0;
const double MID_TO_HIGH_FREQ = 800.0;

const double MID_TO_LOW_FREQ  = 300.0;
const double HIGH_TO_MID_FREQ = 700.0;

// ---------------- Idle Detection ----------------
const int IDLE_LOW_ADC  = 1980;
const int IDLE_HIGH_ADC = 2120;
...
// ============================================================
// Speaker / Manual Filter Test Mode
// ============================================================
void runSpeakerTestMode() {
  setFilter("LOW");

  clearDynamicPanels();
  drawFrequencyPanel(0.0);
  drawClassPanel("LOW");
  drawModePanel("LOW");
  drawStatusLine("TEST MODE: LOW filter active - listen to speaker");
  delay(3000);
  ...
  drawClassPanel("MID");
  drawModePanel("MID");
  drawStatusLine("TEST MODE: MID filter active - listen to speaker");
  delay(3000);
  ...
  drawClassPanel("HIGH");
  drawModePanel("HIGH");
  drawStatusLine("TEST MODE: HIGH filter active - listen to speaker");
  delay(3000);
}
...
void loop() {
  ...
  if (smoothedPreADC > IDLE_LOW_ADC && smoothedPreADC < IDLE_HIGH_ADC) {
    newClass = "LOW";
    currentClass = "LOW";
  }
  else {
  ...
  drawStatusLine("A1 feeds TDA2822 speaker amp. LOW is default.");
